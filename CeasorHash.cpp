    ...
    */
    ...
    class WyHashSecret{ // Original: https://github.com/wangyi-fudan/wyhash/blob/master/wyhash.h
        // For Wyhash 'secret maker' used in class 'WaterHash' below
        // Isnt it 'too complex' to just transform key???
        ...
        uint64_t _wymix(uint64_t A, uint64_t B){ _wymum(&A,&B); return A^B; }
        uint64_t wyrand(uint64_t *seed){ *seed+=0x2d358dccaa6c78a5ull; return _wymix(*seed,*seed^0x8bb84b93962eacc9ull);}
    ...
    public:
        std::vector<uint32_t> GetHashSeq(const uint32_t keyXOR, const int maskSize){
            std::vector<uint32_t> mask(maskSize, 0);

            // Transform Key
            uint64_t secret[4] = {0, 0, 0, 0};
            WyHashSecret wyHashSecret;
            wyHashSecret.make_secret(keyXOR, secret); // keyXOR is uint32_t -> uint64_t upcasting

            int counter = 0;
            for(int i = 0; i < maskSize; i++){
                mask[i] = waterhash(secret, 4, seed ^ (counter+i));
            }
    ...
    private:
        ...
        std::vector<maskN> GenerateMask(const std::vector<dataN>& data, const std::vector<dataN>& key, int maskSize){
            ...
            WaterHash waterHash{};
            std::vector<maskN> mask = waterHash.GetHashSeq(keyXOR, maskSize); // generate wyHash sequence with key
